...
bool type_value_signed_literal_fits_negated(sf_value_type type,
                                            uint64_t magnitude) {
  switch (type) {
  case SF_VAL_TYPE_I8:
...
bool type_value_is_same_group(sf_value_type a, sf_value_type b) {
  if (type_value_is_unsigned(a) && type_value_is_unsigned(b))
    return true;
  if (type_value_is_signed(a) && type_value_is_signed(b))
    return true;
...
bool type_value_is_castable(sf_value_type from, sf_value_type to) {
  if ((from == SF_VAL_TYPE_BOOL) != (to == SF_VAL_TYPE_BOOL))
    return false;
...
sf_opcode type_operation_to_opcode(sf_operation_type type) {
  ...
  case SF_OP_TYPE_BITWISE_AND:
    ...
    break;
  case SF_OP_TYPE_BITWISE_OR:
    op = SF_OPCODE_BITWISE_OR;
    break;
  case SF_OP_TYPE_BITWISE_XOR:
    op = SF_OPCODE_BITWISE_XOR;
    break;
  case SF_OP_TYPE_BITWISE_RSHIFT:
    op = SF_OPCODE_BITWISE_RSHIFT;
    break;
  case SF_OP_TYPE_BITWISE_LSHIFT:
    op = SF_OPCODE_BITWISE_LSHIFT;
    break;
  case SF_OP_TYPE_BITWISE_NOT:
