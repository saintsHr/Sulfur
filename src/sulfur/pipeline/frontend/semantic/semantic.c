...
static bool analyze_expr(
    sf_ast_node* node,
    sf_value_type expected,
    sf_scope* scope,
    const char* filename
);
static void analyze_statement(
    sf_ast_node* node, sf_scope* scope, const char* filename
);
...
static void report_undeclared(
    const char* name, sf_scope* scope, sf_span span, const char* filename
);

static bool check_assignment_type(
    sf_value_type resolved,
    sf_value_type target,
    sf_span span,
    const char* filename
);
...
static bool analyze_expr(
    ...
    switch (node->type) {
        case SF_NODE_BINARY_EXPR: {
            ...
            if (ltype == SF_VAL_TYPE_UNRESOLVED ||
        ...
        case SF_NODE_CAST_EXPR: {
            ...
            if (!analyze_expr(
                    cast->operand, SF_VAL_TYPE_UNRESOLVED, scope, filename
                ))
                return false;
...
static void analyze_statement(
    ...
    switch (node->type) {
        ...
        case SF_NODE_VAR_ASSIGN: {
            ...
            if (!check_assignment_type(
                    asg->value->resolved, sym->type, asg->base.span, filename
                ))
                break;
