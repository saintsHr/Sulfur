...
static bool sf_apply_relational_s(sf_opcode opcode, int64_t l, int64_t r,
                                  bool *result) {
  switch (opcode) {
...
static bool sf_apply_relational_u(sf_opcode opcode, uint64_t l, uint64_t r,
                                  bool *result) {
  switch (opcode) {
  case SF_OPCODE_RELATIONAL_EQUAL:
    *result = l == r;
...
static bool sf_apply_arith_s(sf_opcode opcode, int64_t l, int64_t r,
                             int64_t *result) {
  switch (opcode) {
  case SF_OPCODE_ADD:
    *result = l + r;
...
static bool sf_apply_arith_u(sf_opcode opcode, uint64_t l, uint64_t r,
                             uint64_t *result) {
  switch (opcode) {
  case SF_OPCODE_ADD:
    *result = l + r;
...
static bool sf_apply_bitwise_s(sf_opcode opcode, int64_t l, int64_t r,
                               int64_t *result) {
  switch (opcode) {
...
static bool sf_apply_bitwise_u(sf_opcode opcode, uint64_t l, uint64_t r,
                               uint64_t *result) {
  switch (opcode) {
  case SF_OPCODE_BITWISE_AND:
    *result = l & r;
...
bool sf_fold_constants(sf_arena *arena, sf_operand left, sf_operand right,
                       sf_opcode opcode, sf_value_type result_type,
                       sf_operand *out) {
  bool is_arithmetic = (opcode == SF_OPCODE_ADD || opcode == SF_OPCODE_SUB ||
                        opcode == SF_OPCODE_MULT || opcode == SF_OPCODE_DIV);

  bool is_relational = (opcode == SF_OPCODE_RELATIONAL_EQUAL ||
  ...
  bool is_bitwise =
      (opcode == SF_OPCODE_BITWISE_AND || opcode == SF_OPCODE_BITWISE_OR ||
       opcode == SF_OPCODE_BITWISE_XOR || opcode == SF_OPCODE_BITWISE_RSHIFT ||
  ...
  if (!is_unary && right.type != SF_OPERAND_TYPE_IMMEDIATE)
    return false;
  ...
  if (is_relational) {
    ...
    if (is_signed) {
      ok = sf_apply_relational_s(
          opcode, strtoll(left.immediate_value, NULL, 10),
          strtoll(right.immediate_value, NULL, 10), &result);
    } else {
      ok = sf_apply_relational_u(
          opcode, strtoull(left.immediate_value, NULL, 10),
          strtoull(right.immediate_value, NULL, 10), &result);
    }

    if (!ok)
      return false;

    *out = (sf_operand){.type = SF_OPERAND_TYPE_IMMEDIATE,
  ...
  if (is_bitwise) {
    ...
    if (is_signed) {
      ...
      ok =
          sf_apply_bitwise_s(opcode, strtoll(left.immediate_value, NULL, 10),
                             strtoll(right.immediate_value, NULL, 10), &result);
      ...
      ok = sf_apply_bitwise_u(opcode, strtoull(left.immediate_value, NULL, 10),
  ...
  if (is_unary) {
    ...
    if (is_signed) {
      ...
      ok = sf_apply_unary_s(opcode, strtoll(left.immediate_value, NULL, 10),
                            &result);
      ...
      ok = sf_apply_unary_u(opcode, strtoull(left.immediate_value, NULL, 10),
                            &result);
  ...
  if (is_signed) {
    ...
    ok = sf_apply_arith_s(opcode, strtoll(left.immediate_value, NULL, 10),
                          strtoll(right.immediate_value, NULL, 10), &result);
    ...
    ok = sf_apply_arith_u(opcode, strtoull(left.immediate_value, NULL, 10),
                          strtoull(right.immediate_value, NULL, 10), &result);
